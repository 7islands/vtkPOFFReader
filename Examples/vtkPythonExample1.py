...
# A simple VTK-Python scripting example for use with vtkPOFFReader.
#
...
#     caseType: 0 = decomposed case (appended), 1 = reconstructed case,
#               2 = decomposed case (multiblock)
# Ex.:
...
if (len(sys.argv) < 3):
  ...
  print '  caseType: 0 = decomposed case (appended), 1 = reconstructed case, 2 = decomposed case (multiblock)'
  quit()
...
try:
  reader = vtk.vtkPOFFDevReader() # This package
except AttributeError:
  reader = vtk.vtkPOpenFOAMReader() # VTK 5.6 now has the reader
...
# caseType: 0/1/2 = Decomposed-appended/Reconstructed/decomposed-multiblock case
reader.SetCaseType(caseType)
# Alternatively, one can set case type by one of (but only works with
# this package):
# reader.SetCaseTypeToDecomposedCaseAppended()
# reader.SetCaseTypeToReconstructedCase()
# reader.SetCaseTypeToDecomposedCaseMultiblock()
